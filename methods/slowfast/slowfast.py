...
__all__ = ['get_slowfast']


def get_slowfast(config):
    backbone = config.get('backbone', None)
    num_class = config.get('num_class', None)
    dropout = config.get('dropout', None)
    if backbone == "resnet50":
        return resnet50_slowfast(class_num=num_class, dropout=dropout)
    else:
...
if __name__ == "__main__":
    ...
    model = resnet50_slowfast(class_num=num_classes)
