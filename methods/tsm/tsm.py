...
def get_tsm(config):
    # change backbones from 'resnet50' to 'resnet50_tsm'
    backbone = config['backbone'] + "_tsm"
    return TSN(backbone=backbone, num_class=config['num_class'],
...
class TSN(nn.Module):
    ...
    def _prepare_tsn(self, num_class):
        ...
        print("feature_dim is {}".format(feature_dim))
    ...
    def train(self, mode=True):
        ...
        if self._enable_pbn:
            ...
            for m in self.base_model.modules():
                ...
                if isinstance(m, torch.nn.BatchNorm2d) or isinstance(m,linklink.nn.syncbn_layer.SyncBatchNorm2d):
                    count += 1
