...
def get_3d_backbone(backbone, pretrained, **kwargs):
    if 'resnet' in backbone:
        ...
        elif '152' in backbone:
            model = resnet.resnet152(**kwargs)
        else:
            raise NotImplementedError
    elif 'resnext' in backbone:
        # cardinality in kwargs
        if '50' in backbone:
            model = resnext.resnet50(**kwargs)
        elif '101' in backbone:
            model = resnext.resnet101(**kwargs)
        elif '152' in backbone:
            model = resnext.resnet152(**kwargs)
        else:
            raise NotImplementedError
    elif 'pre_act_resnet' in backbone:
        if '50' in backbone:
            model = pre_act_resnet.resnet50(**kwargs)
        elif '101' in backbone:
            model = pre_act_resnet.resnet101(**kwargs)
        elif '152' in backbone:
            model = pre_act_resnet.resnet152(**kwargs)
        else:
            raise NotImplementedError
    else:
        raise NotImplementedError
...
def generate_model(opt):
    """This is the original design of 3d backbone
    """
    assert opt.model in [
